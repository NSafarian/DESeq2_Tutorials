...
dds <- DESeqDataSetFromMatrix(countData = cts, 
                              colData = coldata, 
                              design = 
                                ~ Ethnicity+Reported_Gender+PMI+ageOfDeath) 
                                
# Note: if you'd like to include ONLY numerical covariates (such as Age) in the model, you'll need to be careful of INTERCEPT term. # Since, models with or without intercept (i.e., Design= ~ Age vs. Design= ~0+Age , respectively) will yeild different outputs. 
# Plotting the fitted line for each gene and checking the sploes is one way to make sure which model is describing the data better. # Though, as a role of thumb, for numerical covariates it'd be best to include the intercept (for example, Design = ~ Age). 
# IF you have multiple factors and numerical covariates you don't need to worry about this issue. 
...
ggplot(G1, aes(x=ageOfDeath, y=count)) + 
  ...
  theme_bw()+
  theme(axis.text.x = element_blank(), # this will erase the X-axis labels 
...
sigs.df$diffexpressed[sigs.df$log2FoldChange > 0.5] <- "UP"
sigs.df$diffexpressed[sigs.df$log2FoldChange < - 0.5] <- "DOWN"
